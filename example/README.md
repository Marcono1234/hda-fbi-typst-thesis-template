...
`thesis-template` contains the content of the template, that is, the content of [`template/`](/template/) in the root directory of this repository.
...
> [!NOTE]\
> To mimic how real usage of the template would look like, this example includes the template in the `thesis-template` directory. However, to avoid code duplication in this repository, that directory is simply a symlink with target [`../template/`](../template/).\
This symlink might cause problems when cloning the Git repository, especially on Windows (see related [Stack Overflow answer](https://stackoverflow.com/a/59761201)). If this causes issues for you, simply delete the `thesis-template` file / directory, create a new directory with that name and copy into it the contents of [`template/`](/template/) in the root directory of this repository (Git might report untracked files then, but for this example that can be ignored).
